...
local function getDefenseValue_kel(rAttacker, rDefender, rRoll) -- luacheck: ignore
    ...
    local sDefenderNodeType, nodeDefender =
        ActorManager.getTypeAndNode(rDefender);
    ...
    if sDefenderNodeType == "pc" then
        if rRoll.sType == "attack" then
            ...
            nFlatFootedMod = nDefense -
                                 DB.getValue(nodeDefender,
                                             "ac.totals.flatfooted", 10);
            nTouchMod = nDefense -
                            DB.getValue(nodeDefender, "ac.totals.touch", 10);
        else
            nDefense = DB.getValue(nodeDefender, "ac.totals.cmd", 10);
            nFlatFootedMod =
                DB.getValue(nodeDefender, "ac.totals.general", 10) -
                    DB.getValue(nodeDefender, "ac.totals.flatfooted", 10);
        end
    ...
    elseif sDefenderNodeType == "npc" then
        if rRoll.sType == "attack" then
            ...
            if sFlatFootedAC then
                ...
                nFlatFootedMod = ActorManager35E.getAbilityBonus(rDefender,
                                                                 sDefenseStat);
            end
    ...
    nDefenseStatMod = ActorManager35E.getAbilityBonus(rDefender, sDefenseStat) +
                          ActorManager35E.getAbilityBonus(rDefender,
                                                          sDefenseStat2);
    ...
    if ActorManager.hasCT(rDefender) then
        ...
        if EffectManager35E.hasEffect(rAttacker, "Ethereal", rDefender, true,
                                      false, rRoll.tags) then
            nBonusSituational = nBonusSituational - 2;
            if not ActorManager35E.hasSpecialAbility(rDefender, "Uncanny Dodge",
                                                     false, false, true) then
                bCombatAdvantage = true;
            end
        elseif EffectManager35E.hasEffect(rAttacker, "Invisible", rDefender,
                                          true, false, rRoll.tags) then
            local bBlindFight = ActorManager35E.hasSpecialAbility(rDefender,
                                                                  "Blind-Fight",
                                                                  true, false,
                                                                  false);
            if sAttackType == "R" or not bBlindFight then
                nBonusSituational = nBonusSituational - 2;
                if not ActorManager35E.hasSpecialAbility(rDefender,
                                                         "Uncanny Dodge", false,
                                                         false, true) then
                    bCombatAdvantage = true;
                end
        ...
        if EffectManager35E.hasEffect(rAttacker, "CA", rDefender, true, false,
                                      rRoll.tags) then
            bCombatAdvantage = true;
        elseif EffectManager35E.hasEffect(rDefender, "GRANTCA", rAttacker,
                                          false, false, rRoll.tags) then
            bCombatAdvantage = true;
        end
        -- END
        if EffectManager35E.hasEffect(rDefender, "Blinded", nil, false, false,
                                      rRoll.tags) then
            nBonusSituational = nBonusSituational - 2;
            bCombatAdvantage = true;
        end
        if EffectManager35E.hasEffect(rDefender, "Cowering", nil, false, false,
                                      rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Rebuked", nil, false, false,
                                       rRoll.tags) then
            nBonusSituational = nBonusSituational - 2;
            bCombatAdvantage = true;
        end
        if EffectManager35E.hasEffect(rDefender, "Slowed", nil, false, false,
                                      rRoll.tags) then
            nBonusSituational = nBonusSituational - 1;
        end
        -- KEL adding uncanny dodge
        if ((EffectManager35E.hasEffect(rDefender, "Flat-footed", nil, false,
                                        false, rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Flatfooted", nil, false,
                                       false, rRoll.tags)) and
            not ActorManager35E.hasSpecialAbility(rDefender, "Uncanny Dodge",
                                                  false, false, true)) or
            EffectManager35E.hasEffect(rDefender, "Climbing", nil, false, false,
                                       rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Running", nil, false, false,
                                       rRoll.tags) then
            bCombatAdvantage = true;
        end
        if EffectManager35E.hasEffect(rDefender, "Pinned", nil, false, false,
                                      rRoll.tags) then
            bCombatAdvantage = true;
            if bPFMode then
                nBonusSituational = nBonusSituational - 4;
            else
                if not EffectManager35E.hasEffect(rAttacker, "Grappled", nil,
                                                  false, false, rRoll.tags) then
                    nBonusSituational = nBonusSituational - 4;
                end
            end
        elseif not bPFMode and
            EffectManager35E.hasEffect(rDefender, "Grappled", nil, false, false,
                                       rRoll.tags) then
            if not EffectManager35E.hasEffect(rAttacker, "Grappled", nil, false,
                                              false, rRoll.tags) then
                bCombatAdvantage = true;
            end
        end
        if EffectManager35E.hasEffect(rDefender, "Helpless", nil, false, false,
                                      rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Paralyzed", nil, false,
                                       false, rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Petrified", nil, false,
                                       false, rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Unconscious", nil, false,
                                       false, rRoll.tags) then
            if sAttackType == "M" then
                nBonusSituational = nBonusSituational - 4;
            end
        ...
        if EffectManager35E.hasEffect(rDefender, "Kneeling", nil, false, false,
                                      rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Sitting", nil, false, false,
                                       rRoll.tags) then
            if sAttackType == "M" then
                nBonusSituational = nBonusSituational - 2;
            elseif sAttackType == "R" then
        ...
        elseif EffectManager35E.hasEffect(rDefender, "Prone", nil, false, false,
                                          rRoll.tags) then
            if sAttackType == "M" then
                nBonusSituational = nBonusSituational - 4;
            elseif sAttackType == "R" then
        ...
        if EffectManager35E.hasEffect(rDefender, "Squeezing", nil, false, false,
                                      rRoll.tags) then
            nBonusSituational = nBonusSituational - 4;
        end
        if EffectManager35E.hasEffect(rDefender, "Stunned", nil, false, false,
                                      rRoll.tags) then
            nBonusSituational = nBonusSituational - 2;
            if rRoll.sType == "grapple" then
                nBonusSituational = nBonusSituational - 4;
        ...
        -- KEL Ethereal
        if EffectManager35E.hasEffect(rDefender, "Invisible", rAttacker, false,
                                      false, rRoll.tags) or
            EffectManager35E.hasEffect(rDefender, "Ethereal", rAttacker, false,
                                       false, rRoll.tags) then
            bTotalConceal = true;
        end
        -- END
        ...
        local aACEffects = EffectManager35E.getEffectsBonusByType(rDefender,
                                                                  {"AC"}, true,
                                                                  aAttackFilter,
                                                                  rAttacker,
                                                                  false,
                ...
                if sBonusType then
                    ...
                    elseif aExistingBonusByType[sBonusType] then
                        ...
                        elseif v.mod > aExistingBonusByType[sBonusType] then
                            nBonusAC = nBonusAC + v.mod -
                                           aExistingBonusByType[sBonusType];
        ...
        local aACCCEffects = EffectManager35E.getEffectsBonusByType(rDefender,
                                                                    {"ACCC"},
                                                                    true,
                                                                    aAttackFilter,
                                                                    rAttacker,
        ...
        -- END
        if rRoll.sType == "grapple" then
            local nPFMod, nPFCount = EffectManager35E.getEffectsBonus(rDefender,
                                                                      {"CMD"},
                                                                      true,
                                                                      aAttackFilter,
                                                                      rAttacker,
        ...
        local nBonusStat1 = ActorManager35E.getAbilityEffectsBonus(rDefender,
                                                                   sDefenseStat,
                                                                   rRoll.tags);
        if (sDefenderNodeType == "pc") and (nBonusStat1 > 0) then
            if DB.getValue(nodeDefender, "encumbrance.armormaxstatbonusactive",
                           0) == 1 then
                local nCurrentStatBonus =
                    ActorManager35E.getAbilityBonus(rDefender, sDefenseStat);
                local nMaxStatBonus = math.max(
                                          DB.getValue(nodeDefender,
                                                      "encumbrance.armormaxstatbonus",
                                                      0), 0);
                local nMaxEffectStatModBonus =
                    math.max(nMaxStatBonus - nCurrentStatBonus, 0);
                if nBonusStat1 > nMaxEffectStatModBonus then
                    nBonusStat1 = nMaxEffectStatModBonus;
                end
            end
        end
        if not bFlatFooted and not bCombatAdvantage and sDefenseStat ==
            "dexterity" then
            nFlatFootedMod = nFlatFootedMod + nBonusStat1;
        end
        nBonusStat = nBonusStat + nBonusStat1;
        local nBonusStat2 = ActorManager35E.getAbilityEffectsBonus(rDefender,
                                                                   sDefenseStat2,
                                                                   rRoll.tags);
        if not bFlatFooted and not bCombatAdvantage and sDefenseStat2 ==
            "dexterity" then
            nFlatFootedMod = nFlatFootedMod + nBonusStat2;
        end
        nBonusStat = nBonusStat + nBonusStat2;
        local nBonusStat3 = ActorManager35E.getAbilityEffectsBonus(rDefender,
                                                                   sDefenseStat3,
                                                                   rRoll.tags);
        if not bFlatFooted and not bCombatAdvantage and sDefenseStat3 ==
            "dexterity" then
        ...
        if rRoll.sType == "grapple" then
            local nNegLevelMod, nNegLevelCount =
                EffectManager35E.getEffectsBonus(rDefender, {"NLVL"}, true, nil,
                                                 nil, false, rRoll.tags);
        ...
        if nCover < 8 then
            local aCover = EffectManager35E.getEffectsByType(rDefender,
                                                             "SCOVER",
                                                             aAttackFilter,
                                                             rAttacker, false,
                                                             rRoll.tags);
            if #aCover > 0 or
                EffectManager35E.hasEffect(rDefender, "SCOVER", rAttacker,
                                           false, false, rRoll.tags) then
                nBonusSituational = nBonusSituational + 8 - nCover;
            elseif nCover < 4 then
                aCover = EffectManager35E.getEffectsByType(rDefender, "COVER",
                                                           aAttackFilter,
                                                           rAttacker, false,
                                                           rRoll.tags);
                if #aCover > 0 or
                    EffectManager35E.hasEffect(rDefender, "COVER", rAttacker,
                                               false, false, rRoll.tags) then
                    nBonusSituational = nBonusSituational + 4 - nCover;
                elseif nCover < 2 then
                    aCover = EffectManager35E.getEffectsByType(rDefender,
                                                               "PCOVER",
                                                               aAttackFilter,
                                                               rAttacker, false,
                                                               rRoll.tags);
                    if #aCover > 0 or
                        EffectManager35E.hasEffect(rDefender, "PCOVER",
                                                   rAttacker, false, false,
                                                   rRoll.tags) then
        ...
        local aVConcealEffect, aVConcealCount =
            EffectManager35E.getEffectsBonusByType(rDefender, "VCONC", true,
        ...
        if not (nMissChance >= 50) then
            local aConceal = EffectManager35E.getEffectsByType(rDefender,
                                                               "TCONC",
                                                               aAttackFilter,
                                                               rAttacker, false,
                                                               rRoll.tags);
            if #aConceal > 0 or
                EffectManager35E.hasEffect(rDefender, "TCONC", rAttacker, false,
                                           false, rRoll.tags) or bTotalConceal or
                bAttackerBlinded then
                nMissChance = 50;
            elseif not (nMissChance >= 20) then
                aConceal = EffectManager35E.getEffectsByType(rDefender, "CONC",
                                                             aAttackFilter,
                                                             rAttacker, false,
                                                             rRoll.tags);
                if #aConceal > 0 or
                    EffectManager35E.hasEffect(rDefender, "CONC", rAttacker,
                                               false, false, rRoll.tags) or
                    bConceal then nMissChance = 20; end
            end
        end

        -- CHECK INCORPOREALITY
        if not bPFMode and (nMissChance < 50) then
            local bIncorporealDefender =
                EffectManager35E.hasEffect(rDefender, "Incorporeal", rAttacker,
                                           false, false, rRoll.tags);
            local bGhostTouchAttacker = EffectManager35E.hasEffect(rAttacker,
                                                                   "ghost touch",
                                                                   rDefender,
                                                                   false, false,
                                                                   rRoll.tags);

            if bIncorporealDefender and not bGhostTouchAttacker and
                not bIncorporealAttack then nMissChance = 50; end
        end
...
local function getDefenseValue_new(rAttacker, rDefender, rRoll)
    ...
    if ActorManager.hasCT(rDefender) then
        ...
        if rRoll.sType == "grapple" then
            local nNegLevelMod, nNegLevelCount =
                EffectManager35E.getEffectsBonus(rDefender, {"NLVL"}, true);
            if nNegLevelCount > 0 then
                nBonusSituational = nBonusSituational - nNegLevelMod;
            end
        end
        ...
        if nCover < 8 then
            local aCover = EffectManager35E.getEffectsByType(rDefender,
                                                             ...
                                                             rAttacker);
            if #aCover > 0 or
                EffectManager35E.hasEffect(rDefender, "SCOVER", rAttacker) then
                nBonusSituational = nBonusSituational + 8 - nCover;
            elseif nCover < 4 then
                aCover = EffectManager35E.getEffectsByType(rDefender, "COVER",
                                                           aAttackFilter,
                                                           rAttacker);
                if #aCover > 0 or
                    EffectManager35E.hasEffect(rDefender, "COVER", rAttacker) then
                    nBonusSituational = nBonusSituational + 4 - nCover;
                elseif nCover < 2 then
                    aCover = EffectManager35E.getEffectsByType(rDefender,
                                                               "PCOVER",
        ...
        -- GET DEFENDER SITUATIONAL MODIFIERS - CONCEALMENT
        local aConceal = EffectManager35E.getEffectsByType(rDefender, "TCONC",
                                                           aAttackFilter,
                                                           rAttacker);
        if #aConceal > 0 or
            EffectManager35E.hasEffect(rDefender, "TCONC", rAttacker) or
            bTotalConceal or bAttackerBlinded then
            nMissChance = 50;
        else
            aConceal = EffectManager35E.getEffectsByType(rDefender, "CONC",
                                                         aAttackFilter,
                                                         rAttacker);
            if #aConceal > 0 or
                EffectManager35E.hasEffect(rDefender, "CONC", rAttacker) or
                bConceal then nMissChance = 20; end
        end
