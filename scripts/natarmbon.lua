...
local function getDefenseValue_kel(rAttacker, rDefender, rRoll) -- luacheck: ignore
	...
	if ActorManager.hasCT(rDefender) then
		...
		if
			EffectManager35E.hasEffect(rDefender, 'Cowering', nil, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Rebuked', nil, false, false, rRoll.tags)
		then
			nBonusSituational = nBonusSituational - 2
			bCombatAdvantage = true
		end
		...
		if
			(
				(
					EffectManager35E.hasEffect(rDefender, 'Flat-footed', nil, false, false, rRoll.tags)
					or EffectManager35E.hasEffect(rDefender, 'Flatfooted', nil, false, false, rRoll.tags)
				) and not ActorManager35E.hasSpecialAbility(rDefender, 'Uncanny Dodge', false, false, true)
			)
			or EffectManager35E.hasEffect(rDefender, 'Climbing', nil, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Running', nil, false, false, rRoll.tags)
		then
			bCombatAdvantage = true
		end
		...
		if
			EffectManager35E.hasEffect(rDefender, 'Helpless', nil, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Paralyzed', nil, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Petrified', nil, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Unconscious', nil, false, false, rRoll.tags)
		then
			if sAttackType == 'M' then nBonusSituational = nBonusSituational - 4 end
			bZeroAbility = true
		end
		if
			EffectManager35E.hasEffect(rDefender, 'Kneeling', nil, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Sitting', nil, false, false, rRoll.tags)
		then
			if sAttackType == 'M' then
				nBonusSituational = nBonusSituational - 2
			elseif sAttackType == 'R' then
		...
		-- KEL Ethereal
		if
			EffectManager35E.hasEffect(rDefender, 'Invisible', rAttacker, false, false, rRoll.tags)
			or EffectManager35E.hasEffect(rDefender, 'Ethereal', rAttacker, false, false, rRoll.tags)
		then
			bTotalConceal = true
		end
		-- END
		...
		local aVConcealEffect, aVConcealCount =
			EffectManager35E.getEffectsBonusByType(rDefender, 'VCONC', true, aAttackFilter, rAttacker, false, rRoll.tags)
		...
		if nMissChance < 50 then
			...
			if
				#aConceal > 0
				or EffectManager35E.hasEffect(rDefender, 'TCONC', rAttacker, false, false, rRoll.tags)
				or bTotalConceal
				or bAttackerBlinded
			then
				nMissChance = 50
...
local function getDefenseValue_new(rAttacker, rDefender, rRoll)
	...
	if ActorManager.hasCT(rDefender) then
		...
		if
			EffectManager35E.hasEffect(rDefender, 'Flat-footed')
			or EffectManager35E.hasEffect(rDefender, 'Flatfooted')
			or EffectManager35E.hasEffect(rDefender, 'Climbing')
			or EffectManager35E.hasEffect(rDefender, 'Running')
		then
			bCombatAdvantage = true
		end
		...
		if
			EffectManager35E.hasEffect(rDefender, 'Helpless')
			or EffectManager35E.hasEffect(rDefender, 'Paralyzed')
			or EffectManager35E.hasEffect(rDefender, 'Petrified')
			or EffectManager35E.hasEffect(rDefender, 'Unconscious')
		then
			if sAttackType == 'M' then nBonusSituational = nBonusSituational - 4 end
			bZeroAbility = true
		end
